...
import { EmailTextOverride, Stos } from 'entities';
import importTexts from './text.json';
...
export type EmailTextsLatestVersion = {
  emailTexts: StoEmailTexts | undefined;
  stoEmailTexts: StoEmailTexts[] | [] | undefined;
};
...
export class EmailTextsService {
  /**
   * Global storage for email texts which mimics multisto code
   * Had no time to refactor it as a cache service
   */
  static global: GlobalEmailTextsConfig = {
    config: {
      emailTexts: undefined,
      stoEmailTexts: undefined,
    },
  };

  static globalEmailTexts(): StoEmailTexts | undefined {
    return EmailTextsService.global.config.emailTexts;
  }

  static globalStoEmailTexts(): StoEmailTexts[] | [] | undefined {
    return EmailTextsService.global.config.stoEmailTexts;
  }

  protected getKeys = async (locale: string = 'en'): Promise<EmailTextOverrides[]> => {
    const keys = await EmailTextOverride.find({ locale });
    return keys.map((key) => ({
      ...key,
      // This is a hack and doesn't conform to the correct type (string) from Schema
      value: key.value === 'text' || key.value?.startsWith('[') ? JSON.parse(key.value) : key.value,
    }));
  };
  ...
  protected getEmailKeyValue = (
    ...
    const value =
      keys.find((k) => k.emailKey === emailTitle && k.key === emailKey && k.stoID === stoID)
        ?.value ??
      keys.find((k) => k.emailKey === emailTitle && k.key === emailKey && k.stoID === 0)?.value ??
  ...
  /**
   * Find and compile one email object out of a bunch of keys
   */
  protected emailFromKeys = (
    ...
    const email: EmailTexts = {
      Subject: getEmailKeyValue(emailTitle, 'Subject', keysUsed, stoID) as string | undefined,
      subject: getEmailKeyValue(emailTitle, 'subject', keysUsed, stoID) as string | undefined,
      Line1: getEmailKeyValue(emailTitle, 'Line1', keysUsed, stoID) as string | undefined,
      Line2: getEmailKeyValue(emailTitle, 'Line2', keysUsed, stoID) as string | undefined,
      Line3: getEmailKeyValue(emailTitle, 'Line3', keysUsed, stoID) as string | undefined,
      // This one is always string[] and I'm parsing it with JSON.parse when getting it from the DB
      text: (getEmailKeyValue(emailTitle, 'text', keysUsed, stoID) as string[]) ?? undefined,
    };
    return email;
  };

  /**
   * Takes a repository array of sto emails, returns a new one with added/modified index at given sto ID with given emails
   */
  protected addStoEmailTexts = (
    ...
    stoEmailTexts[stoID] = stoEmailTexts[0] ? { ...stoEmailTexts[0] } : importTexts; // Copy-Coalesce whole email objects
  ...
  protected getStoIDs = async (): Promise<number[]> => {
    ...
    return stos.map((sto) => sto.ID).filter((id) => id !== 0);
  };

  /**
   * Updates email texts with their latest version
   * @param setStoID Not mandatory, but the default email texts object (emailTexts) will be reconfigured for this STO.
   * @param locale Optionally choose language if available in the overrides, default 'en'.
   */
  public updateEmailTexts = async (setStoID: number = 0, locale: string = 'en'): Promise<void> => {
    const { addStoEmailTexts, getStoIDs, getKeys } = this;
    // Load all texts for all stos

    /** All raw email field-value records */
    const allKeys = await getKeys(locale);
    ...
    stoIDs.forEach((stoID) => {
      stoEmailTexts = addStoEmailTexts(stoEmailTexts, allKeys, emailTitles, stoID);
    });

    EmailTextsService.global.config.emailTexts = stoEmailTexts[setStoID ?? 0];
    EmailTextsService.global.config.stoEmailTexts = stoEmailTexts;
  };
  ...
  public overrideEmailTexts = async (
    ...
    try {
      keys.map(async (key) => {
        const emailTextOverride = await EmailTextOverride.findOne({
          emailKey: emailTitle,
          key,
          stoID,
          locale,
        });
        if (emailTextOverride) {
          await EmailTextOverride.update(emailTextOverride.ID, {
            value: emailFields[key as keyof EmailTexts] as string,
          });
        } else {
          const newEmailTextOverride = EmailTextOverride.create({
            emailKey: emailTitle,
            key,
            value: emailFields[key as keyof EmailTexts] as string,
            stoID,
          ...
          await newEmailTextOverride.save();
        }
      });
      return true;
    } catch (error) {
