...
const MyBuyOfferActionCell: React.FC<Cell<InvestorExchangeOffer>> = ({ row: { original } }) => {
  ...
  if (status === AtomicSwapStatus.Processed) {
    return (
      <>
        <Label>{t('tradingComponentsMySellOffersSwapCompleteMessage')}</Label>
        <Button size="sm" onClick={handleDelete}>
          {t('tradingComponentsMySellOffersDeleteOfferButton')}
        </Button>
      </>
    );
  }
  ...
  return (
    <>
      <Button size="sm" onClick={handleDetails}>
        {swapInProgress
          ? t('tradingComponentsMySellOffersViewSwapButton')
          : t('tradingComponentsMySellOffersViewSwapInProgressButton')}
      </Button>
      <Button size="sm" onClick={handleDelete}>
        {t('tradingComponentsMyBuyOffersDeleteOfferButton')}
      </Button>
