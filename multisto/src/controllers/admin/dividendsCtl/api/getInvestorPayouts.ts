...
const validateRequest = (req: any, res: Response): boolean => {
  ...
  if (!req.params.id || Number.isNaN(+req.params.id)) {
    res
      .status(400)
      .send({ message: "Missing URL param 'id' for dividend payout ID" });
    return false;
  }
  return true;
};
export default async (req: any, res: Response) => {
  try {
    if (validateRequest(req, res)) {
      ...
      const payoutRecs = await svc.listInvestorPayouts(stoId, id);
      ...
      // console.log(`DIVIDEND-INVESTOR PAYOUTS / ${id}`);
      // console.log(payouts);
