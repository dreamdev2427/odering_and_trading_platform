...
const queryParamsReducer = (acc: any[], val: SqlQuery<any>): any[] => [
    ...acc,
    ...val.params,
];
const getIndex = (index: number) => <A>(arr: A[]): A => arr[index];
...
export default class SqlQuery<A> implements PromiseLike<A[]> {
    ...
    then<TResult1 = A[], TResult2 = never>(
        ...
        if (!this._result)
            this._result = this.connection<A>(this.query, this.params);
     ...
     * @returns Promise
     */
    static all = async <B extends SqlQuery<any>[]>(
        ...args: B
    ): Promise<ExtractQueryTypes<B>> => {
        if (args.length === 0)
            // @ts-ignore ignored because typescript cannot infer array is empty
            return [];
        const query = args.reduce(queryStringReducer, "");
        const params = args.reduce(queryParamsReducer, []);
        const chainResultP = args[0].connection(query, params) as Promise<
            ExtractQueryTypes<B>
        >;

        // set result in separate queries in case queries
        args.forEach((q, index) =>
            q.setResult(chainResultP.then(getIndex(index)))
        );
        return chainResultP;
    };
}
