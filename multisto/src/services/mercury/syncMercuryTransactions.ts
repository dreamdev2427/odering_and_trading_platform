...
const { insert, update, executeTransaction } = require("../../modules/db");
...
const isNoteValid = (note: string | null): boolean =>
  noteRegex.test(note || "");

const selectValidNote = (transaction: MercuryTransaction): string | null => {
  // eslint-disable-next-line no-nested-ternary
  if (isNoteValid(transaction.note)) {
    return transaction.note;
  }
  if (isNoteValid(transaction.externalMemo)) {
    return transaction.externalMemo;
  }
  if (isNoteValid(transaction.bankDescription)) {
    return transaction.bankDescription;
  }
  return null;
};
...
): MercuryTransaction & TargetWallet => ({
  ...transaction,
...
const containsNotePredicate = (transaction: MercuryTransaction) =>
  ...
  isNoteValid(transaction.bankDescription);

const checkInvestor = async (
  transaction: MercuryTransaction & TargetWallet
): Promise<boolean> => {
  logger.error(`checkInvestor`);
  logger.error(JSON.stringify(transaction));
  const [investor] = await findMany<Investor>(
    "SELECT * FROM investor where ID=?;",
    [transaction.investorID]
  );
  logger.error(`investor`, investor);

  const [sto] = await findMany<Stos>("SELECT * FROM stos where ID=?;", [
    transaction.stoID,
  ]);
  logger.error(`sto`, sto);
...
const updateBalance = async (
  ...
  executeTransaction(async (connection: PoolConnection) => {
    logger.error(`updateBalance`);
    logger.error(JSON.stringify(transaction));
    // check if idempotency key already exists
    const existingTransaction = findMany<InvestorDepositReceivedAlert>(
...
const statusFilter = (status: TransactionStatus) => (
  transaction: MercuryTransaction
) => transaction.status === status;

const updateLatestTransactionSQL = (
...
const updateLatestTransaction = (
  ...
  const pending = transactions
    ...
    .map(getCreatedAt);
  if (pending.length > 0) {
    return updateLatestTransactionSQL(
      currentConfig,
      new Date(Math.min(...pending))
    );
  }
...
export default async (config: MercuryConfig): Promise<void> => {
  const startDate: string = queryDateFormat(
    new Date(config.lastProcessedTransaction)
  );
  ...
  try {
    ...
    logger.error("error processing mercury transactions");
